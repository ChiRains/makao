			                	...
			                	<c:choose>
			                		...
			                		<c:when test="${executorType.key eq 2}">
										...
										<tbody>
											...
											<tr>
												<td>${departmentMap[item.departmentId].displayName}</td>
												<td>${clerkMap[item.clerkId].name}</td>
												<td class="center">
													<label>
														<input type="checkbox" class="ace select-member-trigger" name="characterMemberIds" value="${item.clerkId}"
														<c:forEach items="${clerkIdList}" var="processExecutor" varStatus="current">
															<c:if test="${processExecutor.memberId eq item.clerkId && processExecutor.type eq executorType.key}">
																checked
															</c:if>
														</c:forEach>/>
														<span class="lbl"></span>
