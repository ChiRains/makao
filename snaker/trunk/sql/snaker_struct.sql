...
CREATE TABLE snaker_task_actor (
    task_Id           VARCHAR(32) not null comment '任务ID',
    actor_Id          VARCHAR(50) not null comment '参与者ID'
)comment='任务参与者表';

create table snaker_hist_order (
    id                VARCHAR(32) not null primary key comment '主键ID',
    process_Id        VARCHAR(32) not null comment '流程定义ID',
...
create table snaker_hist_task_actor (
    task_Id           VARCHAR(32) not null comment '任务ID',
    actor_Id          VARCHAR(50) not null comment '参与者ID'
)comment='历史任务参与者表';

create table snaker_surrogate (
    id                VARCHAR(32) PRIMARY KEY NOT NULL COMMENT '主键ID',
...
create index IDX_SURROGATE_OPERATOR on snaker_surrogate (operator);

create table snaker_cc_order (
...
create index IDX_CCORDER_ORDER on snaker_cc_order (order_Id);

create index IDX_PROCESS_NAME on snaker_process (name);
create index IDX_ORDER_PROCESSID on snaker_order (process_Id);
create index IDX_ORDER_NO on snaker_order (order_No);
create index IDX_TASK_ORDER on snaker_task (order_Id);
create index IDX_TASK_TASKNAME on snaker_task (task_Name);
create index IDX_TASK_PARENTTASK on snaker_task (parent_Task_Id);
create index IDX_TASKACTOR_TASK on snaker_task_actor (task_Id);
create index IDX_HIST_ORDER_PROCESSID on snaker_hist_order (process_Id);
create index IDX_HIST_ORDER_NO on snaker_hist_order (order_No);
create index IDX_HIST_TASK_ORDER on snaker_hist_task (order_Id);
create index IDX_HIST_TASK_TASKNAME on snaker_hist_task (task_Name);
create index IDX_HIST_TASK_PARENTTASK on snaker_hist_task (parent_Task_Id);
create index IDX_HIST_TASKACTOR_TASK on snaker_hist_task_actor (task_Id);

alter table snaker_task_actor
  add constraint FK_TASK_ACTOR_TASKID foreign key (task_Id)
  references snaker_task (id);
alter table snaker_task
  add constraint FK_TASK_ORDERID foreign key (order_Id)
  references snaker_order (id);
alter table snaker_order
  add constraint FK_ORDER_PARENTID foreign key (parent_Id)
  references snaker_order (id);
alter table snaker_order
  add constraint FK_ORDER_PROCESSID foreign key (process_Id)
  references snaker_process (id);
alter table snaker_hist_task_actor
  add constraint FK_HIST_TASKACTOR foreign key (task_Id)
  references snaker_hist_task (id);
alter table snaker_hist_task
  add constraint FK_HIST_TASK_ORDERID foreign key (order_Id)
  references snaker_hist_order (id);
alter table snaker_hist_order
