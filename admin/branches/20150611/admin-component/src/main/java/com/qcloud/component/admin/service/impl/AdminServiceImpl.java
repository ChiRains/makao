...
public class AdminServiceImpl implements AdminService {
	...
	public void init() {
		if (StringUtils.isEmpty(account)
				|| "${pirates.superman.account}".equals(account)) {
			throw new AdminException("请设置超级管理员账号.");
		}
		if (StringUtils.isEmpty(password)
				|| "${pirates.superman.password}".equals(password)) {
		...
		if (str == null || str.indexOf(perAccountCode) == -1) {
			String value = str == null ? perAccountCode : str
					+ PermissionClient.NO_GRANT_ACCOUNT_CODES_KEY_SPLIT
					+ perAccountCode;
			parameterClient.reg(PermissionClient.NO_GRANT_ACCOUNT_CODES_KEY,
	...
	public boolean isSuperman(String account, String password) {
		return this.account.equals(account)
				&& this.password.equals(EncryptUtil.md5(password));
	}
	...
	public boolean isAdmin(String account, String password) {
		...
		if (admin != null
				&& admin.getEnable() == AdminEnableType.DISABLE.getKey()) {
			throw new AdminException("管理员已被禁用." + admin.getAccount());
		}
		return admin != null
				&& EncryptUtil.md5(password).equals(admin.getPassword());
	}
