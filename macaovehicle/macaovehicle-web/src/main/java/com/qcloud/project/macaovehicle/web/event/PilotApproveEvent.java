...
public class PilotApproveEvent implements FormEvent {
    ...
    public void doEvent(FormEventType type, EventContext context) {
        ...
        if (StringUtils.isEmpty(context.getProcessInstId()) && StringUtils.isEmpty(context.getWorkitemId())) {
            ...
            String driverIds = (String) context.getParameter("drivers");
            logger.info("vehicle" + vehicleId);
            logger.info("drivers" + driverIds);
            AssertUtil.assertTrue(StringUtils.isNotEmpty(vehicleId) || StringUtils.isNotEmpty(driverIds), "车辆和驾驶员必须申请至少一个.");
            //
            ...
            context.addReturnResult("progressType", String.valueOf(ProgressType.TJJSY.getKey()));
            context.addReturnResult("person.qc_inner_number", "1");
