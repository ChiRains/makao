								        ...
								        <div>
								            <div class="dataTables_wrapper form-inline no-footer">
								                <div class="row">		
								                 <div class="col-xs-4">
								                 	 	<div class="dataTables_length">
								                            <a title="@她" class="btn btn-sm btn-info"
								                               href="/callMe/toAdd.do">
								                ...
								                <table class="table table-striped table-bordered table-hover dataTable no-footer">
								                    ...
								                    <tbody>
								                           <c:forEach items="${result}" var="call" varStatus="current"> 
								                            <tr>								                                                     
									                            <td>${call.fromSeaman}</td>                
									                            <td>${call.toSeaman}</td>         
									                            <td>
									                            	${call.receiveSeaman}
									                            </td>         
									                            <td>${call.subject}</td>
									                            <td>
									                            	<c:if test="${call.state eq 1}">
								                            		 	 <select class="receiveSeaman"  name="receiveSeaman">
								                            		 		<c:forEach items="${memberList}" var="member"> 
								                            		 			<option>${member}</option>
								                            		 		</c:forEach> 
								                            		 	 </select>
								                            		 	 <button class="btn take" data-id="${call.id}" type="button">
										                            		 <i class="ace-icon fa fa-check"></i>
										                            		   接单
									                            		 </button>
									                            	</c:if>
									                            	<c:if test="${call.state eq 2}">
									                            		<button class="btn finished" type="button"  data-id="${call.id}">
										                            		 <i class="ace-icon fa fa-check"></i>
										                            		 完成 
									                            		 </button>
									                            	</c:if>
									                            	<c:if test="${call.state eq 3}">
									                            		<button class="btn closed" type="button" data-id="${call.id}">
										                            		 <i class="ace-icon fa fa-check"></i>
										                            		 关闭
									                            		</button>
									                            	</c:if>
									                            	<c:if test="${call.state eq 4}">
										                            	 问题已关闭
									                            	</c:if>
									                            </td>                 									                          
									                        </tr>
														    ...
														    <ul class='pagination'>
															    ...
															    <c:if test="${hasPrevious==true}">
															        <li class="paginate_button">
															            <a  href="/callMe/list.do?pageNum=1">首页</a>
															        </li> 
															        <li class="paginate_button previous">
															            <a href="/callMe/list.do?pageNum=${previous}">上一页</a>
															        </li> 
															        
															        <c:forEach items="${previousList}" var="item" varStatus="current"> 
																        <li class="paginate_button">
																            <a href="/callMe/list.do?pageNum=${item.pageNum}">${item.pageNum}</a>
																        </li>
															        </c:forEach>        
															    </c:if>
															    
															    <c:if test="${hasPrevious==false}">
															    	<li class="paginate_button disabled"><a href="javascript:;">上一页</a></li>
														    	</c:if>
														    	
														        <li class="paginate_button active"><span class="current">${pageNum}</span>          
														        </li>
														        
														        <c:if test="${hasNext==true}">
															        <c:forEach items="${nextList}" var="item" varStatus="current"> 
																        <li class="paginate_button">
																            <a href="/callMe/list.do?pageNum=${item.pageNum}">${item.pageNum}</a>
																        </li>
															        </c:forEach>  
															        ...
															        <li class="paginate_button next">
															        	<a class="next" href="/callMe/list.do?pageNum=${next}">下一页</a>
															        </li> 
															        <li class="paginate_button">
															        	<a href="/callMe/list.do?pageNum=${last}">末页</a>
															        </li>        
														        </c:if>
														        ...
														        <li class="paginate_button disabled"><a href="javascript:;">共${total}条</a></li>
														    </ul>
														</div>
												 	</div>
								                </div>  
		...
		<script type="text/javascript">
			...
			jQuery(function($) {
				...
				$(".take").click(function(){
					...
					var receiveSeaman=$(this).prev().val();
					 $.ajax({
				    	url : '/callMe/take.do',
				    	data:{id:id,receiveSeaman:receiveSeaman},
				    	type:'post',
				    	dataType:'json',
