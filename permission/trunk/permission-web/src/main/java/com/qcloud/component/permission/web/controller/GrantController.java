...
public class GrantController {
    ...
    @RequestMapping
    public ModelAndView list(HttpServletRequest request, Integer pageNum,String keyword) {
        ...
        if (str != null) {
            String[] filterAccounts = str.split(PermissionClient.NO_GRANT_ACCOUNT_CODES_KEY_SPLIT);
        ...
        List<AccountVO> listByNames=new ArrayList<AccountVO>();
        for (AccountVO accountVO : list) {
            if(accountVO.getCode().contains(keyword)){
                listByNames.add(accountVO);
            }
        }
        PagingView model = new PagingView("permission/grant-list", pageNum, pageSize);
        model.addObject("accountList", listByNames);
    ...
    public ModelAndView toGrant(HttpServletRequest request, Long accountId) {
        ...
        String[] accountStrs = permissionFilterService.getPermissionAccountCodes(request);
        Long[] accounts = null;
        if (accountStrs != null) {
