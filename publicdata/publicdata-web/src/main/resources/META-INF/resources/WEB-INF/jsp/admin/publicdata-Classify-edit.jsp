...
<div class="row">
    <div class="col-xs-12">
        <!-- PAGE CONTENT BEGINS -->
        <form id="model-form" class="form-horizontal"  role="form" action="/admin/classify/edit.do">
            <!-- #section:elements.form -->
			<input type="hidden" name="id" value="${classify.id}">
			<input type="hidden" name="type" value="${classify.type}">
           ...
           <div class="form-group">
					...
					<span class="col-sm-5 no-padding block input-icon input-icon-right mr10">
						<select class="form-control" id="parentId" name="parentId">
	                          ...
	                          <c:forEach items="${classifyList}" var="item" varStatus="current"> 
			                  	 <option value="${item.id}" ${item.selected}>${item.path}</option>
			                  </c:forEach> 
			             </select> 
					</span>
                </div>
            ...
            <div class="form-group">
                ...
                <div class="col-sm-9">
					<span class="col-sm-5 no-padding block input-icon input-icon-right mr10">
						<input type="hidden"  id="image" name="image" value="${classify.imageUid}"/>
						<button type="button" mult="false" sid="image" vid="pic-pic-view"   class="btn btn-sm btn-purple btn-upload-pic"  upfrom="0"  ><i class="ace-icon fa fa-upload"></i> 上 传</button><ul sid="pic" id="pic-pic-view" class="ace-thumbnails clearfix">
							<img style="max-width:150px;max-height:150px;" alt="" src="${classify.image}">
						</ul>
					</span>
                </div>
            </div>
