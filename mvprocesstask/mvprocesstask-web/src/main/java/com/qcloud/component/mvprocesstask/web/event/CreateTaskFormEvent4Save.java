...
@Component
public class CreateTaskFormEvent4Save implements FormEvent {
    ...
    public void doEvent(FormEventType type, EventContext context) {
        ...
        if (workitemList != null && !workitemList.isEmpty()) {
            for (String workitemId : workitemList) {
                ...
                //
                String personTypeStr = (String) context.getParameter("type");
                AssertUtil.assertNotNull(personTypeStr, "申请类型不能为空.");
                Integer personType = Integer.valueOf(personTypeStr);
            ...
            if (true) {
                String ownerName = (String) context.getParameter("person[0].name");
                String companyName = (String) context.getParameter("enterprisers[0].company");
                String companyCode = (String) context.getParameter("enterprisers[0].code");
                if (!StringUtils.isEmpty(ownerName) || !StringUtils.isEmpty(companyName) || !StringUtils.isEmpty(companyCode)) {
                    Tasking tasking = taskingService.getByWorkitem(context.getWorkitemId());
                    tasking.setOwnerName(!StringUtils.isEmpty(ownerName) ? ownerName : tasking.getOwnerName());
                    tasking.setCompanyName(!StringUtils.isEmpty(companyName) ? companyName : tasking.getCompanyName());
                    tasking.setCompanyCode(!StringUtils.isEmpty(companyCode) ? companyCode : tasking.getCompanyCode());
